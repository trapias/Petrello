 ...
 * - 1.7:
 *   - set reminders for timeline pins (one 15 and one 5 minutes before expiry)
 */
var UI = require('ui');
      ...
      for (var b = 0; b < data.length; b++) {
        ...
        if (theCard.due !== undefined && theCard.due !== null && theCard.due !== "") {
          ...
          if (cardDate >= pastTwoDays && cardDate <= nextYear) {
            var reminderTime = new Date(cardDate.getFullYear(), cardDate.getMonth(), cardDate.getDate(), cardDate.getHours(), cardDate.getMinutes() - 15, cardDate.getSeconds());
            var reminderTime2 = new Date(cardDate.getFullYear(), cardDate.getMonth(), cardDate.getDate(), cardDate.getHours(), cardDate.getMinutes() - 5, cardDate.getSeconds());
            ...
            var pin = {
              ...
              "reminders": [{
                ...
                "layout": {
                  ...
                  "body": theCard.name + ' expires in 5 minutes',
                ...
                "layout": {
                  ...
                  "title": theCard.name,
                  "subtitle": "Petrello reminder",
                  "body": theCard.name + ' expires in 15 minutes',
                  "tinyIcon": "system://images/NOTIFICATION_REMINDER"
                }
              }],
              "actions": [{
  ...
  ajax({
    ...
    function(data, status, request) {
      ...
      for (var b = 0; b < data.length; b++) {
        ...
        if (theBoard.closed === true) {
          continue;
        } // only show open boards
        if (theBoard.organization !== undefined) {
          if (onlyPriv === true) {
            continue;
          }
          if (singleOrgID !== null && theBoard.organization.id !== singleOrgID) {
            continue;
          }
      ...
      for (var o = 0; o < organizations.length; o++) {
        for (b = 0; b < data.length; b++) {
          ...
          if (aBoard.organization !== undefined) {
            ...
            if (singleOrgID !== null && aBoard.organization.id !== singleOrgID) {
              continue;
            }
...
function ShowCardMenu(card) {
  ...
  var cardID = card.id,
    checkItems = card.badges.checkItems; //, members = card.members;
